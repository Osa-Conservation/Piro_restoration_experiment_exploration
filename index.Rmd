...
#Download de biomass database

timeseries_results <- read.csv("data/Piro_Planet_timeseries_AGB_update_2014-2024.csv", header=T)
...
fig <- plot_ly(width = "100%", height = 300) %>%
  layout(
...
survey_table %>%
  ...
  add_header_above(c(" " = 1, "Number of unique site-date surveys by type and year" = ncol(survey_table)-1))
...
# Create a treemap
plot_treemap <- bc_2025_sum %>%
  arrange(desc(sample_frequency)) %>%
  plot_ly(
    type = "treemap",
    labels = ~common_name,
    values = ~sample_frequency,
    parents = NA,  # No hierarchy, just flat structure
    textinfo = "label+value+percent root",
    texttemplate = "<b>%{label}</b><br>Frequency: %{value}<br>%{percentRoot:.1%}",
    hovertemplate = "<b>%{label}</b><br>Detection Frequency: %{value}<br>Percentage: %{percentRoot:.1%}<extra></extra>",
    marker = list(
      colors = ~sample_frequency,
      colorscale = "Blues",
      showscale = FALSE,
      colorbar = list(title = "Detection Frequency")
    )
  ) %>%
  layout(
    title = "Species Detection Frequency",
...
leaflet(data = bat_first) %>%
  addPolygons(data=plots, popup=plots$plot_id, group="plots") %>%
  addProviderTiles(providers$Esri.WorldImagery, group = "Satellite") %>%
  addProviderTiles(providers$OpenStreetMap, group = "Street Map") %>%  addCircleMarkers(
    lng = ~longitude,
    lat = ~latitude,
    radius = 3,
    color = "darkred",
...
In total, we detected `r sum(amphib_caps_2025$count)` individuals of `r length(unique(amphib_caps_2025$species))` different species. The distribution of species detected was as follows: 
...
# Assuming your full amphibian captures data is called amphib_caps
# and has a date column and species column

# Convert date if needed
amphib_caps$date <- as.Date(amphib_caps$date)
amphib_caps$year <- year(amphib_caps$date)

# Split into pre-2025 and 2025
pre_2025 <- amphib_caps %>%
  filter(year < 2025) %>%
  count(species, name = "detections") %>%
  arrange(detections) %>%
  filter(species!="NA") %>% 
  filter(species!="Dendropsophus craugastor") %>% 
  mutate(species = factor(species, levels = species))

post_2025 <- amphib_caps %>%
  filter(year >= 2025) %>%
  count(species, name = "detections") %>%
  arrange(detections) %>%
  filter(species!="NA") %>% 
...
# Combine data for consistent species ordering
all_species <- union(pre_2025$species, post_2025$species)

pre_2025_complete <- data.frame(species = all_species) %>%
  left_join(pre_2025, by = "species") %>%
  mutate(detections = ifelse(is.na(detections), 0, detections))

post_2025_complete <- data.frame(species = all_species) %>%
  left_join(post_2025, by = "species") %>%
  mutate(detections = ifelse(is.na(detections), 0, detections))

# Order by total detections
species_order <- pre_2025_complete %>%
  left_join(post_2025_complete, by = "species", suffix = c("_pre", "_post")) %>%
  mutate(total = detections_pre + detections_post) %>%
  arrange(total) %>%
  pull(species)

pre_2025_complete <- pre_2025_complete %>%
  mutate(species = factor(species, levels = species_order))

post_2025_complete <- post_2025_complete %>%
  mutate(species = factor(species, levels = species_order))


# Prepare data for butterfly chart
pre_data <- pre_2025_complete %>%
  mutate(detections_butterfly = -detections,  # Negative for left side
         period = "Pre-2025")

post_data <- post_2025_complete %>%
  mutate(detections_butterfly = detections,
         period = "2025")

# Combine data
butterfly_data <- bind_rows(pre_data, post_data)

# Create butterfly chart
plot_ly(butterfly_data,
        y = ~species,
        x = ~detections_butterfly,
        color = ~period,
        type = "bar",
        orientation = "h",
        colors = setNames(plasma_colors[1:2], c("Pre-2025", "2025")),
        height = 800) %>%
  layout(
    title = list(text = "Amphibian Detections: Pre-2025 vs 2025", 
                 y = 0.98),
    xaxis = list(
      title = "Detections",
      tickvals = seq(-max(abs(butterfly_data$detections_butterfly)), 
                     max(abs(butterfly_data$detections_butterfly)), 
                     length.out = 5),
      ticktext = abs(seq(-max(abs(butterfly_data$detections_butterfly)), 
                         max(abs(butterfly_data$detections_butterfly)), 
                         length.out = 5))
    ),
    yaxis = list(title = "Species"),
    barmode = "overlay",
    margin = list(l = 150, t = 80, r = 50),
...
Previous work by Arianna Basto created lists of mammal, bird and amphibian species in the Osa region which are of conservation concern. We will import these lists and extract any species detected thus far in our surveys. 
...
all_caps <- list()
# Create a list of species detected
all_caps[[1]] <- gsub("\\.", " ", sp_summary$sp)
all_caps[[2]] <- unique(bird_counts$species)
all_caps[[3]] <- unique(amphib_caps$species[amphib_caps$code!="NA"])
...
# Create formatted version with NA handling
iucn_formatted <- iucn_all %>%
  mutate(
    IUCN_status_clean = ifelse(is.na(IUCN_status), "Not Assessed", IUCN_status),
    bg_color = ifelse(IUCN_status_clean %in% names(iucn_colors), 
                      iucn_colors[IUCN_status_clean], 
                      "#FFFFFF"),
    text_color = ifelse(IUCN_status_clean %in% names(iucn_text_colors), 
                        iucn_text_colors[IUCN_status_clean], 
                        "black"),
    IUCN_status_display = cell_spec(
      IUCN_status_clean,
      background = bg_color,
      color = text_color,
      bold = TRUE
  ...
  select(-IUCN_status, -IUCN_status_clean, -bg_color, -text_color) %>%
  rename(IUCN_status = IUCN_status_display)

# Table 1: All species
iucn_formatted %>%
  kable(format = "html", escape = FALSE,
        caption = "IUCN status by taxon", row.names = FALSE) %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"),
                full_width = FALSE) %>%
  row_spec(0, bold = TRUE, background = "#f5f5f5")
...
# Make sure iucn_statuses is defined
status_order <- c("Critically Endangered", "Endangered", "Vulnerable", 
                  "Near Threatened", "Least Concern", "Data Deficient", 
                  "Not Assessed", "Unknown")

iucn_statuses <- intersect(status_order, unique(iucn_for_grouping$IUCN_status_clean))

# Create HTML with collapsible sections
html_output <- "<div style='font-family: Arial, sans-serif;'>"

for(status in iucn_statuses) {
  
  # Filter data for this status
  status_data <- iucn_for_grouping %>%
    filter(IUCN_status_clean == status) %>%
    select(scientific_name, Endemism, class_name, IUCN_population_trend)
  
  if(nrow(status_data) == 0) next
  
  # Get colors
  bg_color <- iucn_colors[status]
  text_color <- iucn_text_colors[status]
  
  # Create collapsible section
  html_output <- paste0(html_output, "
    <details style='margin-bottom: 10px; border: 1px solid #ddd; border-radius: 5px;'>
      <summary style='
      ...
      '>
        ", status, " <span style='float: right;'>(", nrow(status_data), " species) ▼</span>
      </summary>
  ...
  status_table <- status_data %>%
    mutate(
      IUCN_population_trend = cell_spec(
        IUCN_population_trend,
        background = case_when(
          IUCN_population_trend == "Increasing" ~ "#e8f5e8",
          IUCN_population_trend == "Stable" ~ "#fff3e0",
          IUCN_population_trend == "Decreasing" ~ "#ffe8e8",
          TRUE ~ "white"
        ),
        color = case_when(
          IUCN_population_trend == "Increasing" ~ "#2e7d32",
          IUCN_population_trend == "Stable" ~ "#b26a00",
          IUCN_population_trend == "Decreasing" ~ "#c62828",
          TRUE ~ "black"
        )
      )
    ) %>%
    kable(format = "html", escape = FALSE,
          col.names = c("Scientific Name", "Endemism", "Class", "Population Trend"),
          align = c("l", "l", "l", "c")) %>%
    kable_styling(bootstrap_options = c("striped", "hover"), 
                  full_width = TRUE, position = "left") %>%
    row_spec(0, bold = TRUE, background = "#f5f5f5")
  
  html_output <- paste0(html_output, as.character(status_table), "
      </div>
    </details>
  ")
}
...
Some sites have low grass cover, but it doesn't appear to be linked to the original restoration methodology (L,M,H,N). High seems to be the worst, and medium the best!
...
leaflet(ll_sf) %>% 
  ...
  addCircleMarkers(
    ...
    stroke = FALSE, 
    fillOpacity = 0.8,
    popup = ~paste(plot_id, "<br> Leaflitter cover:", round(leaf_cover,3))
  ) %>% 
  addLegend(
    pal = pal, 
    values = ~leaf_cover, 
...
# Summarizing data for next steps:

Our monitoring process from the first 8 years of the restoration provide a robust information about the responses of wildlife on the first  most-critical moment on recovery. 

From now on, the surveys can be performed in fewer plots, covering the current trends of recovery.

## We can see the trends from satellite imagery.

Based on Planet's satellite capabilities, we can use biomass and other satellite imagery to monitor restoration recovery by establishing a historical baseline and tracking changes over time. High-frequency, daily imagery from satellites allows for the continuous observation of vegetation regrowth of specific recovery areas, providing quantifiable evidence of restoration success.

For example, if we compare the Biomass (Mg/ha) per site through time, we can clearly see its increase since 2017, right when the restoration began. However, it is still lower than the biomass in pristine forest so the restoration is still on-course.
...
timeseries_results <- timeseries_results %>%
  mutate(plot_id = case_when(
    # For L, H, M, N codes that are 2 characters (letter + single digit)
    grepl("^[LHMN][1-9]$", plot_id) ~ paste0(substr(plot_id, 1, 1), "0", substr(plot_id, 2, 2)),
    # For L, H, M, N codes that are 3 characters (letter + two digits)
    grepl("^[LHMN]10$", plot_id) ~ plot_id,
    # Keep everything else as is
    TRUE ~ plot_id
  ))

# Reshape for plotting
ts_long <- timeseries_results %>%
  pivot_longer(
    cols = c(biomass, canopy_cover, canopy_height),
    names_to = "metric",
    values_to = "value"
  ) %>% 
 # Add Morpho column
  mutate(
    Area = case_when(
      grepl("^P", plot_id) ~ "Forest",
      grepl("^F", plot_id) ~ "Forest",
      grepl("^N", plot_id) ~ "Restoration",
      grepl("^L", plot_id) ~ "Restoration",
      grepl("^H", plot_id) ~ "Restoration",
      grepl("^M", plot_id) ~ "Restoration",
      TRUE ~ "Other"
    ))
...
p1 <- ts_long[ts_long$metric == "biomass", ] %>%
  plot_ly(
    x = ~year,
    y = ~value,
    color = ~Area,
    colors = c("forestgreen", "sienna"),
    split = ~plot_id,  # This creates separate lines for each plot_id
    type = 'scatter',
    mode = 'lines+markers',
    text = ~paste('Plot:', plot_id, '<br>Year:', year, '<br>Value:', round(value, 2)),
    hoverinfo = 'text',
    line = list(width = 2),  # Consistent line width
    marker = list(size = 4)   # Consistent marker size
  ) %>%
  layout(
    title = "Biomass (Mg/ha) - All Areas",
...
Lets make a close-up to the restoration sites. See that the trends are not related to our initial classification H-M-L-N. So the response per each site is very uneven. 

Those are the expected responses to restoration, the heterogenity on soil, flooding, among others makes the tree growing varies in space.

```{r}
# Create second interactive plot (Restoration only)
p2 <- ts_long %>%
  filter(metric == "biomass", Area == "Restoration") %>%
  plot_ly(
    x = ~year,
    y = ~value,
    color = ~plot_id,
  ...
  layout(
    title = "Biomass (Mg/ha) - Restoration Only",
    xaxis = list(title = "Year"),
    yaxis = list(title = "Value"),
    showlegend = FALSE
  )
...
## Lets take a look on how the Biomass spread along our study area


```{r}
#============================================
# CALCULATE DELTA BIOMASS (2020 to 2023)
# ============================================

# Get 2020 values
biomass_2017 <- timeseries_results %>%
  filter(year == 2017) %>%
  select(plot_id, biomass_2017 = biomass)

# Get 2023 values
biomass_2024 <- timeseries_results %>%
  filter(year == 2024) %>%
  select(plot_id, biomass_2024 = biomass)

# Calculate delta
delta_biomass <- biomass_2017 %>%
  inner_join(biomass_2024, by = "plot_id") %>%
  mutate(
    delta = biomass_2024 - biomass_2017,
    pct_change = (delta / biomass_2017) * 100
  )

delta_biomass2<-delta_biomass %>% 
  left_join(plot_master, by="plot_id")

###############################################
 # Calculate min and max for legend
max_bm <- max(delta_biomass2$delta, na.rm = TRUE)
min_bm <- min(delta_biomass2$delta, na.rm = TRUE)
mid_bm <- min_cr + ((max_bm - min_bm)/2)
      
# Create custom HTML legend
legend_html <- paste0(
        '<div style="padding: 10px; background: white; border: 2px solid grey; border-radius: 5px;">',
        '<strong>Biomass</strong><br>',
        '<strong>(Mg/ha)</strong><br><br>',
        '<svg width="120" height="80">',
        '<circle cx="15" cy="15" r="11" fill="rgba(0, 102, 204, 0.6)" />',
        '<text x="35" y="20" font-size="12">High: ', round(max_bm, 2), '</text>',
        '<circle cx="15" cy="40" r="7" fill="rgba(0, 102, 204, 0.6)" />',
        '<text x="35" y="45" font-size="12">Med: ', round(mid_bm, 2), '</text>',
        '<circle cx="15" cy="65" r="3" fill="rgba(0, 102, 204, 0.6)" />',
        '<text x="35" y="70" font-size="12">Low: ', round(min_bm, 2), '</text>',
        '</svg>',
        '</div>'
      )
...
leaflet() %>%
  ...
  addCircleMarkers(
    ...
    lng = ~longitude,
    lat = ~latitude,
    radius = ~(biomass_2024 / max(biomass_2024, na.rm = TRUE) * 10) + 1,
    color = "#7AE69C",
    stroke = FALSE,
    fillOpacity = 0.8,
    popup = ~paste(plot_id, 
                   "<br>Biomass 2017:", round(biomass_2017, 2),
                   "<br>Biomass 2024:", round(biomass_2024,2))
    ) %>%
...
Once the current trends were well established, we can reduce the number of restoration plots to be assessed from now on. Whit this selection, we can perform more effective sampling surveys for the ongoing years.

But, what plots should be keep monitoring? To do so, we checked first the survey locations of bats, amphibians and birds.

```{r}
Piro2<-delta_biomass2 %>% 
  filter(simple_treatment!="forest")

##########################################3
# Create color palettes for each variable
pal_amphibians <- colorFactor(palette = "Reds", domain = Piro2$Amphibians)
pal_bats <- colorFactor(palette = "Blues", domain = Piro2$Bats, na.color = "#808080")
pal_birds <- colorFactor(palette = "Greens", domain = Piro2$Birds)

# For categorical Select variable, use colorFactor with transparent for "No"
pal_select <- colorFactor(
  palette = c("#00000000", "#FF8C00"), # Transparent for "No", dark orange for "Yes"
  domain = Piro2$Select)
...
map <- leaflet(Piro2) %>%
  ...
  # Base location markers (shows all points with basic info)
  addCircleMarkers(
    lng = ~longitude,
    lat = ~latitude,
    popup = ~paste0(
      "<b>Plot:</b> ", plot_id, "<br>",
      "<b>Amphibians:</b> ", sprintf("%.0f", Amphibians), "<br>",
      "<b>Bats:</b> ", ifelse(is.na(Bats), "NA", sprintf("%.0f", Bats)), "<br>",
      "<b>Birds:</b> ", sprintf("%.0f", Birds), "<br>"
    ),
    radius = 3,
    color = "black",
    fillColor = "white",
    fillOpacity = 0.3,
    weight = 1,
    group = "Base Locations",
    label = ~plot_id
  ) %>%
  
  # Amphibians layer (only show counts as integers)
  addCircleMarkers(
    lng = ~longitude,
    lat = ~latitude,
    popup = ~paste0(
    ...
    radius = 8,
    color = "black",
    fillColor = ~pal_amphibians(Amphibians),
    fillOpacity = 0.7,
    weight = 1,
    group = "Amphibians",
    label = ~paste0("Amphibians surveys: ", sprintf("%.0f", Amphibians))
  ) %>%
  
  # Bats layer (handle NAs and show as integers)
  addCircleMarkers(
    lng = ~longitude,
    lat = ~latitude,
    popup = ~paste0(
      "<b>Bats Surveys:</b> ", ifelse(is.na(Bats), "NA", sprintf("%.0f", Bats)), "<br>",
      "<b>Plot:</b> ", plot_id, "<br>"
    ),
    radius = ~ifelse(is.na(Bats), 3, 7),
    color = "black",
    fillColor = ~pal_bats(Bats),
    fillOpacity = 0.7,
    weight = 1,
    group = "Bats",
    label = ~paste0("Bats: ", ifelse(is.na(Bats), "NA", sprintf("%.0f", Bats)))
  ) %>%
  
  # Birds layer (show as integers)
  addCircleMarkers(
    lng = ~longitude,
    lat = ~latitude,
    popup = ~paste0(
    ...
    radius = 6,
    color = "black",
    fillColor = ~pal_birds(Birds),
    fillOpacity = 0.7,
    weight = 1,
    group = "Birds",
    label = ~paste0("Birds: ", sprintf("%.0f", Birds))
  ) %>%
  
  # Add layer controls
  addLayersControl(
    baseGroups = c("Satellite", "Street Map"),
    overlayGroups = c("Base Locations", "Amphibians", "Bats", "Birds"),
    options = layersControlOptions(collapsed = FALSE)
  ) %>%
  
  # Hide some layers by default
  hideGroup(c("Amphibians", "Bats", "Birds")) %>%
  
  # Add legends
  addLegend("bottomright", pal = pal_amphibians, values = ~Amphibians,
            title = "Amphibians", opacity = 0.7, group = "Amphibians") %>%
  addLegend("bottomright", pal = pal_bats, values = ~Bats,
            title = "Bats", opacity = 0.7, group = "Bats") %>%
  addLegend("bottomright", pal = pal_birds, values = ~Birds,
            title = "Birds", opacity = 0.7, group = "Birds") 
...
As you can see, not all surveys were done always in the same locations, so the selected plots must cover the entire recovery and sampling scenarios. 

Here you can compare the Biomass trends between the overall and the selected plots, on Biomass for 2024 and Biomass change between 2017 and 2024.


```{r}
# Create a modified version of the dataset with "Overall" as a new Sampling category

Piroselected<-Piro2 %>% 
  filter(Select != "No") %>% 
  mutate(Sampling = "Selected") 

Piro2_overall <- Piro2 %>%
  filter(simple_treatment !="forest") %>% 
  mutate(Sampling = "Overall") %>% 
  bind_rows(Piroselected)

pos <- position_jitter(width = 0.2, height = 0, seed = 123)

# Create the plot with three violin plots
ggplot(Piro2_overall, aes(x = Sampling, y = biomass_2024)) +
  geom_violin(alpha = 0.3) +
  theme_minimal() +
  labs(x = "Sampling Group", y = "Biomass 2024")+
  geom_point(aes(color = Select),position = pos, size = 3) +
  ggrepel::geom_text_repel(
    aes(label = plot_id),
    size = 3,
    position = pos,  # Use the same position
    max.overlaps = 20,  # Adjust this based on your data density
    force = 0.5,
    segment.size = 0.2,
    segment.alpha = 0.5,
    box.padding = 0.5,
...
# Create the plot with three violin plots
ggplot(Piro2_overall, aes(x = Sampling, y = delta)) +
  geom_violin(alpha = 0.3) +
  theme_minimal() +
  labs(x = "Sampling Group", y = "Delta Biomass 2017-2024")+
  geom_point(aes(color = Select),position = pos, size = 3) +
  ggrepel::geom_text_repel(
    aes(label = plot_id),
    size = 3,
...
Lets see the location of the selected plots.
...
##########################################3
# For categorical Select variable, use colorFactor with transparent for "No"
# Transparent for "No", dark orange for "Yes"

pal_select <- colorFactor(palette = c("#00000000", "#FF8C00"), 
  domain = Piro2$Select)
...
map2 <- leaflet(Piro2) %>%
  ...
  # Select layer (only show "Yes" points prominently)
  addCircleMarkers(
    data = Piro2 %>% filter(Select == "Yes"),
    lng = ~longitude,
    lat = ~latitude,
    popup = ~paste0(
      "<b>SELECTED PLOT</b><br>",
      "<b>Plot:</b> ", plot_id, "<br>",
      "<b>Amphibians:</b> ", sprintf("%.0f", Amphibians), "<br>",
      "<b>Bats:</b> ", ifelse(is.na(Bats), "NA", sprintf("%.0f", Bats)), "<br>",
      "<b>Birds:</b> ", sprintf("%.0f", Birds), "<br>",
    ...
    fillOpacity = 0.9,
    weight = 3,
    group = "Selected Plots",
    label = ~paste0("SELECTED: ", plot_id)
  ) %>%
  
  # Add layer controls
  addLayersControl(
    baseGroups = c("Satellite", "Street Map"),
    overlayGroups = c("Base Locations", "Amphibians", "Bats", "Birds", "Selected Plots"),
    options = layersControlOptions(collapsed = FALSE)
  ) %>%
  
  # Hide some layers by default
  hideGroup(c("Amphibians", "Bats", "Birds", "Selected Plots")) %>%
  
  # Add legends
  addLegend("bottomleft", pal = pal_amphibians, values = ~Amphibians,
            title = "Amphibians", opacity = 0.7, group = "Amphibians") %>%
  addLegend("bottomleft", pal = pal_bats, values = ~Bats,
            title = "Bats", opacity = 0.7, group = "Bats") %>%
  addLegend("bottomleft", pal = pal_birds, values = ~Birds,
            title = "Birds", opacity = 0.7, group = "Birds") %>%
  addLegend("bottomleft", 
            colors = "#FF8C00", 
            labels = "Selected Plots", 
            title = "Selection", 
            opacity = 0.9, 
            group = "Selected Plots")
...
Now, we are ready for our ongoing surveys!
