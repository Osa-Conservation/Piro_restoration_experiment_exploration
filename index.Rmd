...
To date we have deployed `r nrow(dep)` cameras, at `r length(unique(dep$placename))` locations. Resulting in the capture of `r nrow(img[img$is_blank==0,])` wildlife images.
...
plot_data <- capture_rates %>%
  ...
  mutate(
    ...
    simple_treatment = factor(simple_treatment, 
                               levels = c("forest", "active_restoration", "passive_restoration")),
    # Add small value to zeros for log scale, and adjust text color threshold
    capture_rate_log = capture_rate + 0.01,
    text_color = ifelse(capture_rate < 5, "white", "black")
  )

# Create nicer species labels (replace . with space)
plot_data <- plot_data %>%
  mutate(species_label = gsub("\\.", " ", species),
         species_label = factor(species_label, levels = gsub("\\.", " ", rev(forest_order))))

# Create the heatmap with log scale
ggplot(plot_data, aes(x = simple_treatment, y = species_label, fill = capture_rate_log)) +
  geom_tile(color = "white") +
