...
# Create subplots
fig1 <- plot_ly(
  data = pre_2025_complete,
  y = ~species,
  x = ~detections,
  type = "bar",
  orientation = "h",
  marker = list(color = plasma_colors[1]),
  showlegend = FALSE,
  height = 800
) %>%
  layout(
    xaxis = list(title = "Detections"),
    yaxis = list(title = "", showticklabels = TRUE)
  )

fig2 <- plot_ly(
  data = post_2025_complete,
  y = ~species,
  x = ~detections,
  type = "bar",
  orientation = "h",
  marker = list(color = plasma_colors[2]),
...
subplot(fig1, fig2, nrows = 1, shareY = TRUE, titleX = TRUE, widths = c(0.55, 0.45)) %>%
  layout(
    title = list(text = "Amphibian Detections: Pre-2025 vs 2025", y = 0.98),
    margin = list(l = 150, t = 80),
    # Adjust this value as needed (default is ~450)
    annotations = list(
      list(x = 0.25, y = 0.95, text = "<b>Pre-2025</b>", showarrow = FALSE, 
           xref = "paper", yref = "paper", font = list(size = 14), xanchor = "center"),
      list(x = 0.8, y = 0.95, text = "<b>2025</b>", showarrow = FALSE, 
           xref = "paper", yref = "paper", font = list(size = 14), xanchor = "center")
    )
  )
```
...
How do these sites relate to flooded areas? This is an interesting question we can test with tour data.
...
## Lets take a look on hw the Biomass spread along our study area?
...
As you can see, not all surveys were done always in the same locations, so the selected plots can cover the entire recovery and sampling scenarios. 
...
plot_ly(Piro2_overall, 
        x = ~Sampling, 
        y = ~biomass_2024,
        type = 'violin',
        box = list(visible = TRUE),  # Add box plot inside violin
        meanline = list(visible = TRUE),  # Add mean line
        points = FALSE,  # Don't show points in the violin
        spanmode = "hard",  # Keep violins within data range
        name = "Distribution",
        color = I("#A9A9A9"),  # Light gray color
        alpha = 0.3,
        hoverinfo = 'x+y',
        showlegend = FALSE) %>%
  
  # Add points colored by Select
  add_trace(x = ~Sampling, 
            y = ~biomass_2024,
            type = 'scatter',
            mode = 'markers',
            marker = list(size = 8),
            color = ~Select,
            colors = c("firebrick", "forestgreen", "steelblue"),  # Adjust colors as needed
            name = ~Select,
            text = ~paste('Plot:', plot_id, '<br>Biomass:', round(biomass_2024, 2)),
            hoverinfo = 'text',
            showlegend = TRUE) %>%
  
  # Layout customization
  layout(
    title = "Biomass 2024 Distribution by Sampling Group",
    xaxis = list(title = "Sampling Group"),
    yaxis = list(title = "Biomass 2024"),
    violingap = 0.3,  # Gap between violins from different x categories
    violingroupgap = 0.1,  # Gap between violins within the same x category
    violinmode = 'group',  # 'group' for side-by-side, 'overlay' for overlapping
    legend = list(
      title = list(text = "Select"),
